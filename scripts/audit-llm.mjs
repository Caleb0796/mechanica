...
function auditPrompt(data, parts, validation) {
  const quotes = data.sources.map((source) => source.quote).filter((quote) => typeof quote === 'string').join('\n\n')
  return `你是独立的古代机械复原审计员。你只能使用下列古籍原文、最终 parts.json 和确定性验证报告。
...
async function main() {
  ...
  for (const file of machineFiles()) {
    ...
    const partsPath = join(repoRoot, 'src/machines', data.slug, 'parts.json')
    const reportPath = join(repoRoot, 'reports', `${data.slug}.validation.json`)
    if (!existsSync(partsPath) || !existsSync(reportPath)) {
      console.error(`${data.slug}: missing parts.json or validation report`)
      failed = true
      continue
    }
    const parts = JSON.parse(readFileSync(partsPath, 'utf8'))
    const validation = JSON.parse(readFileSync(reportPath, 'utf8'))
    const prompt = auditPrompt(data, parts, validation)
    try {
      const verdict = await callResponses(prompt)
      const artifact = `# ${data.slug} independent audit\n\n- Model: ${sanitize(model)}\n- Generated: ${new Date().toISOString()}\n\n## Verdict\n\n${verdict.trim()}\n`
      writeFileSync(join(outputDirectory, `${data.slug}.md`), sanitize(artifact))
...
if (!apiKey) {
  console.warn('\u001b[33mOPENAI_API_KEY is absent; skipping GPT-5.6 independent audit.\u001b[0m')
} else {
  await main()
}
