...
const machineCards: Record<MachineSlug, MachineCardCopy> = {
  ...
  typecase: {
    ...
    era: { en: 'Northern Song–Yuan · 1041–1298', zh: '北宋至元 · 1041–1298 年' },
    principle: {
        ...
        {MACHINE_SLUGS.map((slug, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="machine-thumbnail">{t('home.thumbnail')}</span>
...
function MachineRoute({ slug }: { slug: string }) {
  ...
  const [module, setModule] = useState<MachineModule | null>(
    slug === 'demo' ? demoModule : null,
  )
  ...
  useEffect(() => {
    ...
    setModule(null)
    const loader = machineModules[`../machines/${slug}/build.ts`]
    if (!loader) {
      setError(true)
      return () => {
        cancelled = true
