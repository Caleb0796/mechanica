...
import type { IKinematicGraph, PartDef, SolveResult } from '../../sim/types'
...
export default function DriveHandle({
  ...
  const drive = (delta: number) => {
    onDrive(graph.drive(part.id, delta))
  }
  ...
  const beginDrag = (event: ThreeEvent<PointerEvent>) => {
    ...
    axis.applyQuaternion(event.object.getWorldQuaternion(event.object.quaternion.clone())).normalize()
  ...
  const continueDrag = (event: ThreeEvent<PointerEvent>) => {
    ...
    if (Math.abs(delta) < 0.0001) {
      delta = (event.nativeEvent.movementX - event.nativeEvent.movementY) * 0.008
    }
