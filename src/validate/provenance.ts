...
function provenance(value: unknown): Provenance | null {
  ...
  return item && (item.kind === 'wenxian' || item.kind === 'wenwu' || item.kind === 'tuice')
...
function nonemptyNote(part: PartDef): boolean {
  ...
  return (part.dimensionNotes ?? []).some((quantity) => hasNote(quantity.provenance))
}

function checkPartProvenance(module: MachineModule, sources: Set<string>): ValidationCheck[] {
  ...
  for (const part of module.spec.parts) {
    ...
    } else if (item.kind === 'tuice' && (!part.schemeTags?.length && !nonemptyNote(part))) {
      status = 'fail'
      message = `Inferred part ${part.id} requires a scheme tag or nonempty dimension note.`
    } else if (item.kind !== 'tuice' && !sources.has(item.ref)) {
      status = 'fail'
      message = `Part ${part.id} references unknown source ${item.ref}.`
    }
    checks.push({ id: `provenance:part:${part.id}`, status, message, sourceRef: item?.ref })
  }
  return checks
}
...
function numericPaths(value: unknown, prefix = ''): string[] {
  ...
  if (Array.isArray(value)) {
    return value.flatMap((entry, index) => numericPaths(entry, prefix ? `${prefix}.${index}` : String(index)))
  }
  const item = record(value)
  if (!item) return []
  return Object.keys(item).sort().flatMap((key) => numericPaths(item[key], prefix ? `${prefix}.${key}` : key))
}
...
function checkNumericGeometry(module: MachineModule, sources: Set<string>): ValidationCheck[] {
  const checks: ValidationCheck[] = []
  for (const part of module.spec.parts) {
    const mapping = record(part.dimensionProvenance) ?? {}
    ...
    for (const path of paths.sort()) {
      ...
      } else if (entry.kind !== 'tuice' && !sources.has(entry.ref)) {
        status = 'fail'
    ...
    const rest = mapping['@rest'] === undefined ? null : provenance(mapping['@rest'])
...
function checkRatioAndDataSources(module: MachineModule, sources: Set<string>): ValidationCheck[] {
  ...
  for (const [index, dimension] of module.data.dimensions.entries()) {
    const ok = dimension.sourceId.trim().length > 0 && sources.has(dimension.sourceId)
    checks.push({
...
function geometryById(module: MachineModule): Map<string, UnknownRecord> {
  return new Map(module.spec.parts.map((part) => [part.id, part.geometry as unknown as UnknownRecord]))
}

function constraintProvenanceRequired(constraint: UnknownRecord): boolean {
  return constraint.type === 'crank' || constraint.type === 'cam' || constraint.type === 'differential'
    || (constraint.type === 'lockstep' && constraint.ratio !== 1)
}

function endpointRadius(geometry: UnknownRecord | undefined): number | null {
  if (!geometry) return null
  if (geometry.type === 'gear' && typeof geometry.module === 'number' && typeof geometry.teeth === 'number') {
    return geometry.module * geometry.teeth / 2
  }
  return geometry.type === 'wheel' && typeof geometry.radius === 'number' ? geometry.radius : null
}

function checkConstraints(module: MachineModule, sources: Set<string>): ValidationCheck[] {
  const checks: ValidationCheck[] = []
  ...
  for (const [index, rawConstraint] of module.spec.constraints.entries()) {
    ...
    if (constraintProvenanceRequired(constraint)) {
      ...
      const ok = Boolean(item) && (item?.kind === 'tuice' || sources.has(item?.ref ?? ''))
      checks.push({
        id: `${id}:provenance`,
        status: ok ? 'pass' : 'fail',
        message: ok
    ...
    if (constraint.type === 'mesh') {
      const a = typeof constraint.a === 'string' ? geometry.get(constraint.a) : undefined
      const b = typeof constraint.b === 'string' ? geometry.get(constraint.b) : undefined
      const ok = a?.type === 'gear' && b?.type === 'gear'
      checks.push({
        id: `${id}:mesh-endpoints`,
        status: ok ? 'pass' : 'fail',
        message: ok ? `Mesh ${index + 1} joins two gears.` : `Mesh ${index + 1} must join two gears.`,
      })
    }
...
const SPECIFIC_TRIGGER_SLUGS = new Set(['astroclock', 'seismoscope', 'odometer', 'loom', 'typecase'])
...
function checkMechanismAndIngenuity(module: MachineModule): ValidationCheck[] {
  ...
  const checks: ValidationCheck[] = [{
    ...
    message: spotlight ? 'Mechanism exposes the spotlight trigger.' : 'Mechanism must expose a spotlight trigger.',
  }]
  ...
  for (const field of ['hook', 'demo', 'echo'] as const) {
    ...
    checks.push({
      ...
      status: ok ? 'pass' : 'fail',
      message: ok ? `Ingenuity ${field} is bilingual.` : `Ingenuity ${field} requires nonempty zh and en text.`,
    })
  }
  return checks
}
...
function repositoryPath(root: string | undefined, file: string): string | null {
  ...
  return prefix ? `${prefix}/${segments.filter((segment) => segment !== '.').join('/')}` : segments.join('/')
}

function checkImages(module: MachineModule, opts: ValidationOptions): ValidationCheck[] {
  const checks: ValidationCheck[] = []
  for (const [index, image] of module.data.images.entries()) {
    ...
    if (image.license === 'linkout') {
      checks.push({
        ...
        message: image.file === undefined
          ? 'Linkout image has no local file.'
          : 'Linkout images must not declare a local file.',
      })
    }
    if ((image.license === 'CC-BY' || image.license === 'CC-BY-SA') && image.file) {
      const ok = Boolean(image.author?.trim() && image.licenseUrl?.trim() && image.attributionText?.trim())
      checks.push({
        id: `${id}:attribution`,
        status: ok ? 'pass' : 'fail',
        message: ok
...
function checkSnapshots(module: MachineModule, opts: ValidationOptions): ValidationCheck[] {
  const checks: ValidationCheck[] = []
