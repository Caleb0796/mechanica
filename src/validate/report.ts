...
function patchedSpec(spec: MachineSpec, patch: SchemePatch): MachineSpec {
  ...
  const overrides = new Map((patch.overrideParts ?? []).map((part) => [part.id, part]))
  const parts = spec.parts
    .filter((part) => !removed.has(part.id))
    .map((part) => ({ ...part, ...(overrides.get(part.id) ?? {}) }))
  ...
  const constraints = spec.constraints.filter((_, index) => !removedConstraints.has(index))
  constraints.push(...(patch.addConstraints ?? []))

  return { ...spec, parts, constraints }
}

export function applySchemePatch(module: MachineModule, patch: SchemePatch): MachineModule {
  return { ...module, spec: patchedSpec(module.spec, patch) }
}
...
function runScope(
  ...
  const module = patch ? applySchemePatch(baseModule, patch) : baseModule
  ...
  try {
    ...
    return {
      checks: [failure(`${scope}:graph`, `Graph construction failed: ${error instanceof Error ? error.message : String(error)}`)],
      resolutionDeg: 0,
    }
  }
  ...
  try {
    checks.push(...validateProvenanceAndIntegrity(module, {
      ...opts,
      allowMissingSnapshots: opts.allowMissingSnapshots || opts.partial,
    }))
  } catch (error) {
    checks.push(failure('integrity', `Integrity validation failed: ${error instanceof Error ? error.message : String(error)}`))
  }

  try {
    checks.push(...validateRatios(module.spec, graph))
  } catch (error) {
    checks.push(failure('ratios', `Ratio validation failed: ${error instanceof Error ? error.message : String(error)}`))
  }
  ...
  try {
    ...
    checks.push(failure('sampling', `Sampling validation failed: ${error instanceof Error ? error.message : String(error)}`))
  }

  return { checks: prefixed(scope, checks), resolutionDeg }
}

export function runValidation(module: MachineModule, opts: ValidationOptions = {}): ValidationReport {
  const results = [
    runScope(module, undefined, opts),
    ...Object.values(module.schemes ?? {}).map((scheme) => runScope(module, scheme, opts)),
  ]
  ...
  const summary = {
    ...
    resolutionDeg: Math.max(0, ...results.map((result) => result.resolutionDeg)),
  }
